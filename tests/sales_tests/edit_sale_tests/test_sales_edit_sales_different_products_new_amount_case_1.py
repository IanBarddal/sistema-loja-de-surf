...
def test_sales_edit_sale_same_product_new_amount_case_1():
    ...
    product_name_entry.get.return_value = "Skate"
    ...
    price_entry.get.return_value = "250"

    new_quantity_entry = MagicMock()
    new_quantity_entry.get.return_value = "2"
    ...
    database.update("UPDATE Produtos SET Estoque = Estoque - 2 WHERE Produto = ?", ["Skate"])
    ...
    result2 = database.read("SELECT * FROM Produtos WHERE Produto = ?", ["Skate"])
    ...
    assert result2[0][1] == "Skate"
    assert result2[0][2] == 250
    assert result2[0][3] == 18